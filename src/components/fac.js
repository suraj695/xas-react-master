  ...
  handlePassword = (name) => {
      ...
      handleSubmit = event => {
	    ...
	    pass:this.state.pass
          ...
          axios.post(`https://xas-python.azurewebsites.net/fac_desk`, { roll })
	.then((res)=>{
		const data = res.data
		console.log(data)
		data.map((i)=>{
			...
			if(i.pass==="true"){
			if(i.name==='Principal')
			window.location="https://xaviers-analytics-system.azurewebsites.net/pal_desk"
			else
			window.location="https://xaviers-analytics-system.azurewebsites.net/fac_desk"
			}
			else
